...
import { allDeletedDocumentsByOwner, hardeleteDocument, restoreDocument } from '../../store/document/actions';
...
const AllDeletedDocumentsByOwner = () => {
    ...
    useEffect(() => {
        dispatch(allDeletedDocumentsByOwner())
    }, [dispatch]);
    ...
    const handleDelete = async () => {
        if (selectedDoc) {
            try {
                ...
                await dispatch(allDeletedDocumentsByOwner()).unwrap();
            } catch (error: any) {
    ...
    const safeItems = Array.isArray(deletedItems) ? deletedItems : [];
    ...
    return (
        <div className="max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8">
            <div className='flex flex-col gap-4 md:flex-row md:justify-between md:items-center mb-3'>
                ...
                <div className='w-full py-2 px-3 bg-white rounded-md shadow-md md:w-auto'>
                    <div className='flex flex-col gap-1 sm:flex-row sm:gap-4'>
                        <h2 className='text-gray-400'>Total documents: <span className='text-gray-800'>{deletedItems.length}</span></h2>
            ...
            <ConfirmationModal
                ...
                message={`Are you sure you want to completely delete ${selectedDoc?.title}? This includes with all datas related to it (versions, comments, and access). This action can't be un done`}
                bgColor="bg-red-600"
...
export default AllDeletedDocumentsByOwner;
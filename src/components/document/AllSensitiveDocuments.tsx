...
import { fetchSensitiveocuments } from '../../store/document/actions';
...
import { ArchiveIcon, Download, DownloadIcon, EyeIcon, Files, MoreVerticalIcon, PencilIcon, ShieldIcon } from 'lucide-react';
import Pagination from '../Pagination';
import { DocumentsSkeletonLoader } from '../SkeletonLoader';

const ITEMS_PER_PAGE = 24;

const AllSensitiveDocuments = ({ onEdit }: { onEdit: (doc: any) => void }) => {
  ...
  useEffect(() => {
    dispatch(fetchSensitiveocuments())
  }, [dispatch])
        ...
        {filteredItems.length === 0 ? (
          <div className=" w-full bg-white flex flex-col items-center p-6 text-center rounded-md justify-center space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-xl font-medium text-gray-900">No sensitive document found</h3>
              <p className="text-gray-500 max-w-md">
                Sensitive documents can't be view by other users, you can set a document sensitive in the document details.
              </p>
            </div>
          </div>
        ) : (
...
export default AllSensitiveDocuments;